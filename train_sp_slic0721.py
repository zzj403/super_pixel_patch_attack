        ...
        for img_name in img_list:
            ...
            with torch.no_grad():
                ...
                for selected_node in select_list:
                    img_now = torch.where(
                        (segments_tensor.repeat(3, 1, 1) == selected_node).mul(mask_detected.repeat(3, 1, 1) == 1), noise_img,
                        img_now)
                    sp_layer_now = torch.where((segments_tensor == selected_node).mul(mask_detected.repeat == 1), white_img_single_layer,
                        sp_layer_now)
                # compute the area
                ...
                for step in range(unique_segments_num):
                    ...
                    boxes222 = do_detect(self.darknet_model, resize_small(img_now).cuda(), 0.4, 0.4, True)
                    class_names = load_class_names('data/coco.names')
                    plot_boxes(transforms.ToPILImage()(img_now.detach().cpu()), boxes222, 'test2/predictions'+str(step)+'.jpg',
                               class_names)
                    print()
                    ...
                    for i1 in range(now_neighbor_unique_np.size):
                        ...
                        osp_img = torch.where(
                            (segments_tensor.repeat(3, 1, 1) == now_node).mul(mask_detected.repeat(3, 1, 1) == 1),
                            noise_img, img_now)
                        osp_img_gpu = resize_small(osp_img).cuda().unsqueeze(0)
                    ...
                    img_now = torch.where(
                        (segments_tensor.repeat(3, 1, 1) == select_superpixel_no).mul(mask_detected.repeat(3, 1, 1) == 1),
                        noise_img, img_now)
                    print()
