    ...
    def train(self):
        ...
        for img_name in img_list:
            ...
            for i in range(3000):
                ...
                noise = torch.Tensor(img_batch_batch.size()).uniform_(-1, 1) * 0.01
                img_batch_batch_noised = img_batch_batch + noise
                patched_img608 = torch.where((super_pixel_batch_608.repeat(3, 3, 1, 1) == 1), adv_patch_cpu_batch, img_batch_batch_noised)
                ...
                tv = self.total_variation(adv_patch_cpu.cuda())
                tv_loss = tv * 2.5
                det_loss = torch.mean(max_prob)
                ...
                if i % 20 == 0:
                    with torch.no_grad():
                        ...
                        img_test = torch.where((super_pixel_batch_500.repeat(3, 1, 1) == 1), adv_patch_cpu_old, img_batch_500)
